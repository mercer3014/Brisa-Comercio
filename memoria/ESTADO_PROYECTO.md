# GEODATA — ESTADO DEL PROYECTO

> Stack: Laravel 12 + Vue 3 (Inertia) + PostgreSQL 16
> Base de datos: `geodata_db` (fuente de verdad — creada por SQL completo en pgAdmin)
...
- `.env`: `DB_CONNECTION=pgsql`, `DB_HOST=127.0.0.1`, `DB_PORT=5432`,
  `DB_DATABASE=geodata_db`, `DB_USERNAME=postgres`, `DB_PASSWORD=awd78951230`,
  `DB_SEARCH_PATH=public`.
  - **Cambio clave:** `DB_DATABASE` estaba en `brisa` (BD inexistente) → corregido a `geodata_db`.
...
Loaders probados con datos reales (`php artisan geodata:probar-etl`):
...
`geodata:refrescar-vistas` referenciaban vistas `resumen_*` que **NO existen** en
`geodata_db`. Las vistas materializadas REALES son:
...
### Paleta Geodata
- Añadida en `resources/css/app.css` (bloque `@theme`) como tokens `--color-geodata-*`
  (navy, red, red-light, blue, green, gold, orange + grises 50/100/200/600/800) y
  `--color-org-{ine,aladi,mercosur,faostat}`. Generan utilidades `bg-/text-/border-geodata-*`.
- Helper JS `resources/js/lib/orgColors.js` (COLORES_ORG, PALETA_GEODATA, colorOrg()).

### Navegación (LayoutPublico.vue) — 8 secciones
Inicio · Organizaciones · Rankings · Comparador · Mapa Comercial · Indicadores ·
Línea de Tiempo · Acerca de · [Acceder]. Logo = `public/img/geodata-mark.svg`.
...
- Footer ya traía logo + «Fuente: INE» + «© Geodata · fines informativos y educativos».
