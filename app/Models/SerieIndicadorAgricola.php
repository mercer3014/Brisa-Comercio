<?php

namespace App\Models;

use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Relations\BelongsTo;

class SerieIndicadorAgricola extends Model
{
    protected $table = 'serie_indicador_agricola';
    protected $primaryKey = 'serie_agricola_id';
    public $incrementing = true;
    protected $keyType = 'int';
    public $timestamps = false;

    protected $guarded = [];

    protected $casts = [
        'valor' => 'decimal:2',
        'atributos_extra' => 'array',
    ];

    public function organizacion(): BelongsTo
    {
        return $this->belongsTo(Organizacion::class, 'organizacion_id', 'organizacion_id');
    }

    public function archivo(): BelongsTo
    {
        return $this->belongsTo(ArchivoFuente::class, 'archivo_id', 'archivo_id');
    }

    public function pais(): BelongsTo
    {
        return $this->belongsTo(Pais::class, 'pais_id', 'pais_id');
    }

    public function productoCodigoExterno(): BelongsTo
    {
        return $this->belongsTo(ProductoCodigoExterno::class, 'producto_codigo_externo_id', 'producto_codigo_externo_id');
    }

    public function tiempo(): BelongsTo
    {
        return $this->belongsTo(Tiempo::class, 'tiempo_id', 'tiempo_id');
    }

    public function elemento(): BelongsTo
    {
        return $this->belongsTo(FaostatElemento::class, 'elemento_id', 'elemento_id');
    }

    public function simbolo(): BelongsTo
    {
        return $this->belongsTo(FaostatSimbolo::class, 'simbolo_id', 'simbolo_id');
    }
}
